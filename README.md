...
### コンテンツのパッキング仕様 (pack_zip.py)

動画再生などの **Range Request (シーク)** を正常に動作させるには、`scripts/pack_zip.py` を使用して ZIP を作成することを推奨します。

- **無圧縮 (STORED)**: `pack_zip.py` は動画や音声などの既圧縮ファイルを無圧縮で ZIP に格納します。ホスト側はこれらをメモリに展開せず、ファイルから直接シークして読み出すため、大容量アセットでも高速・省メモリに動作します。
- **圧縮 (DEFLATED)**: テキストファイル（HTML/JS/CSS等）は圧縮して格納されます。ホスト側はこれらを逐次読み出し（ストリーミング）で提供します。**圧縮エントリはシーク（Range Request）が利用できない**ため、大容量の動画などを外部ツールで圧縮して格納しないよう注意してください。
