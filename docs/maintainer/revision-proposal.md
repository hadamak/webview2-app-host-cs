...
### コネクター起動仕様の改善
- **構造化設定のデフォルト改善**: `web-content/app.conf.json` に `pipe_server` を追加し、デフォルトで外部アタッチ（`--mcp-proxy`）が動作するように修正。
- **エラーフィードバックの強化**: `PipeClientConnector` で接続失敗時に、ホスト側の設定不足（`pipe_server` の欠如）を示唆するログを出力するように改善。
