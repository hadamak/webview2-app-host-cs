...
### 5. 🛡️ コアコンテンツ保護
アプリケーションのエントリポイントや核心ロジックを、外部ファイルによる不用意な上書きから保護します。`.wve`（暗号化）または `.wvc`（平文保護）拡張子を使用することで、**「内側優先」**の読み込みを強制し、改ざんを防ぐことができます。
...
### コンテンツの読み込み優先順
同名ファイルが複数の場所に存在する場合、ホストはその拡張子に応じてどちらを使用するかを決定します。

#### 📄 通常コンテンツ
**外側優先 (Outer-First)**: 外部の `www/` フォルダを最優先することで、開発時アセットの即時反映（ホットリロード的な運用）を可能にします。

1. `www/` フォルダ（EXE 隣接）（最優先）
2. 起動引数で渡された ZIP パス
3. EXE と同名の ZIP（例: `MyApp.exe` に対して `MyApp.zip`）
4. EXE に連結した ZIP
5. EXE 内部に埋め込まれたリソース（最低優先）

#### 🛡️ 保護コンテンツ (`.wve` / `.wvc`)
**内側優先 (Inner-First)**: これらの拡張子を持つファイルには「保護」が適用されます。核心ロジックが外部ファイルによって書き換えられるのを防ぐため、内側のソースが優先されます。

1. EXE 内部に埋め込まれたリソース（最優先）
2. EXE に連結した ZIP
3. EXE と同名の ZIP
4. 起動引数で渡された ZIP パス
5. `www/` フォルダ（最低優先）
...
> [!TIP]
> `index.html` に `.wvc` を使用することで、悪意のある `www/index.html` が置かれたとしても、常に意図したエントリポイントからアプリを起動させることができます。
...
### 5. 🛡️ 保護コンテンツ (.wve / .wvc)
特定の拡張子を持つファイルは「コアコンテンツ」として扱われ、特殊な **「内側優先」** 検索ルールに従います。これにより、ユーザーが `www/` フォルダにファイルを置くことでアプリの安全なエントリポイントを誤って（あるいは意図的に）上書きしてしまうことを防ぎます。

- **`.wve` (暗号化)**: AES-256 で暗号化され、ホストによって透過的に復号されます。秘密にしたいロジックやアセットに適しています。
- **`.wvc` (平文保護)**: 暗号化は行いませんが、強制的に内側優先ルールを適用します。`index.html` などのエントリポイントを保護するのに便利です。

これらの保護ファイルの場合、読み込み優先順位は通常とは **逆に** なります。
1. EXE 内部に埋め込まれたリソース（最優先）
2. EXE に連結した ZIP
3. EXE と同名の ZIP
4. 起動引数で渡された ZIP パス
...
> 🔒 **セキュリティノート:** `.wve` ファイルを生成するには、同梱の暗号化ツール `tools/Protect-File.ps1` を使用してください。
