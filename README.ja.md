...
- 🚀 モジュール化された配布 — 本体を軽量に保ち、必要な拡張機能だけを追加可能
- 🎮 Steam 連携 — JavaScript から実績や統計をアンロック
- ⚡ Node.js サポート — セキュアなサイドカープロセス経由でサーバーサイドロジックを実行
...
### 5. ⚡ Node.js サポート (サイドカー)
ローカルファイルシステムへのアクセスや複雑なサーバーサイド処理が必要ですか？ Node.js サポートコンポーネントを追加することで、ローカルの Node.js サイドカープロセスを実行できます。通信は標準の `postMessage()` / `WebMessageReceived` を介して行われ、Web 開発者に馴染みのあるブリッジを提供します。
